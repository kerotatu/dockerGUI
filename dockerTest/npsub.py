...
def postLines():
    ...
    for e in obj:
        ...
        #print("Address3")
        #print(Address3)
        cmd="tc qdisc add dev eth0 parent 1:{} handle {}: netem delay {}ms".format(ip,10+ip,e["value"])
        #cmd="tc qdisc add dev eth0 parent 1:1 handle 10: netem delay {}ms".format(e["value"])
        container.exec_run(cmd)
        print(cmd)
        #cmd="tc filter add dev eth0 protocol ip parent 1: prio (1) u32 match ip dst 172.16.24.51/32 flowid 1:(1)"
        #cmd="tc filter add dev eth0 protocol ip parent 1: prio 1 u32 match ip dst 172.16.{}.{}/32 flowid 1:1".format(Address3[ip-1],ip)
        cmd="tc filter add dev eth0 protocol ip parent 1: prio {} u32 match ip dst 172.16.{}.{}/32 flowid 1:{}".format(ip,Address3[ip-1],ip,ip)
        print(cmd)
        container.exec_run(cmd)
